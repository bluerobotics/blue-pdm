...
> **Database schema 88.** Ask your admin to re-run `supabase/core.sql`, `supabase/modules/10-source-files.sql` and `supabase/modules/30-supply-chain.sql`. Until they do, BluePLM will report that your database is older than the app — the check-in and RFQ fixes below live in database functions and are not active until those modules are re-run.
